...
export const RemindersScreen: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-white pb-28 w-full relative select-none">
      {/* CLEAN TOP BAR WITHOUT BG COLOR */}
      <div className="bg-transparent pt-4 pb-4 px-4 space-y-3 sticky top-0 z-30">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            <div className="w-10 h-10 rounded-xl bg-teal-50 flex items-center justify-center text-[#0B5A54] ring-1 ring-teal-200/60 shadow-xs">
              <Pill className="w-5 h-5 text-[#0B5A54]" />
            </div>
            <div className="text-left space-y-0.5">
              <h1 className="text-base sm:text-lg font-black text-slate-900 font-heading tracking-tight">Care Reminders</h1>
              <p className="text-[11px] font-semibold text-slate-500">Pill Schedule & Treatment Tracker</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="p-2 px-2.5 rounded-xl bg-teal-50 hover:bg-teal-100 text-[#0B5A54] border border-teal-200 transition-all font-bold text-xs flex items-center gap-1.5 shadow-2xs"
              title="Test Tablet Eating Notification"
            >
              <BellRing className="w-3.5 h-3.5 text-[#0B5A54] animate-pulse" />
              <span className="hidden sm:inline">Test Alert</span>
            </button>

            <button
              onClick={handleAddCustom}
              className="p-2 px-2.5 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-800 border border-slate-200 transition-all font-bold text-xs flex items-center gap-1 shadow-2xs"
            >
          ...
          {reminders.length === 0 ? (
            ...
            reminders.map((rem) => (
              <Card
                key={rem.id}
                padding="md"
                className={`border transition-all duration-150 shadow-2xs ${rem.taken
                  ...
                  <div className="flex flex-col items-end gap-1.5 shrink-0">
                    <div className="flex items-center gap-1.5">
                      <button
                        ...
                        disabled={rem.taken}
                        className={`px-2.5 py-1.5 rounded-xl font-bold text-xs transition-all shadow-xs flex items-center gap-1 ${rem.taken
