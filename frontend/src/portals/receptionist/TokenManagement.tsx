      ...
      >
        ...
        <div className="mt-2.5 flex items-center justify-between gap-3">
          <div className="flex items-center gap-3 min-w-0">
            <div
              className={`w-9 h-9 rounded-full text-white font-black text-xs flex items-center justify-center shadow-xs shrink-0 font-heading ${isOnline
                  ? 'bg-purple-700'
                  : 'bg-[#0B5A54]'
                }`}
            >
            ...
            <div className="min-w-0 space-y-0.5">
              ...
              <p className="text-[11px] text-slate-500 font-medium truncate">
                {(token as any).gender || 'Male'} • {token.age || 30} Years • 📞 {token.patientPhone}
              </p>
          ...
          {/* Time Slot Badge on Right */}
          {token.timeSlot && (
            <div className="shrink-0 bg-emerald-50 text-emerald-800 border border-emerald-200 px-2.5 py-1 rounded-xl text-xs font-mono font-bold">
              {token.timeSlot}
            </div>
          )}
        </div>
  ...
  return (
    <div className="space-y-4 sm:space-y-5 pb-8 text-left w-full max-w-full overflow-x-hidden">
      ...
      <div className="bg-white rounded-2xl sm:rounded-3xl p-4 sm:p-5 border border-slate-200/90 shadow-2xs space-y-4 w-full min-w-0">
        ...
        <div className="space-y-2.5">
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs font-bold text-slate-400">
              {availableTimeSlots.length} Slots Active
            </span>
          </div>
          ...
          >
            ...
            <button
              ...
              className={`p-3 rounded-2xl border-2 text-left transition-all duration-200 cursor-pointer min-w-[170px] max-w-[200px] shrink-0 flex flex-col justify-between ${
                selectedTimeSlot === 'ALL'
                  ? 'bg-white border-[#0B5A54] ring-2 ring-teal-500/20 shadow-xs'
              ...
              <div>
                <div className="flex items-center justify-between gap-1 mb-1">
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <span className="px-2 py-0.5 rounded-full text-[10px] font-mono font-black bg-[#0B5A54] text-white">
                      {filteredTokens.length} Tokens
                    </span>
                  </div>
                </div>
                <h4 className="font-black text-xs text-slate-900 truncate">All Time Slots</h4>
              </div>
            </button>
                  ...
                  <div className="mt-2 pt-1.5 border-t border-slate-100 space-y-1">
                    <div className="flex items-center justify-between gap-1 text-[10px]">
                      ...
                      <span className="inline-flex items-center gap-0.5 text-amber-800 bg-amber-50 px-1 py-0.2 rounded font-bold">
                        ...
                        <span>{metrics.offline} Walk-In</span>
                      </span>
                    </div>
                  </div>
                </button>
        ...
        {/* ══════════════════════════════════════════════════════════════════
            3. DATE FILTER, STATUS FILTER & SORT TOOLBAR (UNIFIED SINGLE ROW)
        ══════════════════════════════════════════════════════════════════ */}
        <div className="pt-2 border-t border-slate-100 w-full min-w-0">
          <div className="flex flex-wrap items-center justify-between gap-2.5 bg-slate-50/80 p-2 sm:p-2.5 rounded-2xl border border-slate-200/70 w-full min-w-0">
            ...
            <div className="flex flex-wrap items-center gap-1.5 min-w-0">
              <span className="text-xs font-bold text-slate-400 mr-0.5">Date:</span>
            ...
            <div className="flex flex-wrap items-center gap-2 shrink-0">
              <div className="flex items-center gap-1 overflow-x-auto no-scrollbar">
                <span className="text-xs font-bold text-slate-400 mr-0.5 shrink-0">
                  Status:
                </span>
              ...
              <div className="relative inline-flex items-center ml-1">
                <select
                  ...
                  className="appearance-none pl-3 pr-7 py-1 bg-white hover:bg-slate-100 border border-slate-200/80 rounded-xl text-xs font-bold text-slate-700 focus:outline-none focus:ring-2 focus:ring-[#0B5A54] shadow-2xs cursor-pointer"
                >
      ...
      <div className="bg-white rounded-2xl p-3.5 sm:p-4 border border-slate-200/90 shadow-xs flex flex-col sm:flex-row sm:items-center justify-between gap-3 transition-all select-none">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-full bg-teal-50 border border-teal-200/80 flex items-center justify-center text-[#0B5A54] shrink-0 shadow-2xs">
        ...
        <div className="flex flex-wrap items-center gap-2 text-xs">
          ...
          {/* Available / Not Available Toggle Button - Only shown when specific slot selected */}
          {selectedTimeSlot !== 'ALL' && (
            <button
      ...
      <div className="space-y-4">
        ...
        <div className="relative flex flex-col md:flex-row items-center justify-center w-full py-1 min-h-[44px] gap-3">
          {/* Centered Division Switcher Tabs (Matching Reference Image) */}
          <div className="inline-flex items-center gap-1.5 bg-slate-100/90 p-1.5 rounded-full border border-slate-200/80 shadow-inner z-10">
            {/* Division 1: App Bookings */}
            <button
              type="button"
              onClick={() => setActiveDivision('ONLINE')}
              className={`px-4 sm:px-5 py-1.5 rounded-full text-xs font-bold transition-all cursor-pointer flex items-center gap-2 ${activeDivision === 'ONLINE'
            ...
            >
              ...
              <span
                className={`px-2 py-0.2 rounded-full text-[10px] font-mono font-bold ${activeDivision === 'ONLINE' ? 'bg-purple-100 text-purple-900' : 'bg-slate-200 text-slate-700'
                  }`}
              >
            ...
            {/* Division 2: Walk-In Desk */}
            <button
              type="button"
              onClick={() => setActiveDivision('OFFLINE')}
              className={`px-4 sm:px-5 py-1.5 rounded-full text-xs font-bold transition-all cursor-pointer flex items-center gap-2 ${activeDivision === 'OFFLINE'
            ...
            >
              ...
              <span
                className={`px-2 py-0.2 rounded-full text-[10px] font-mono font-bold ${activeDivision === 'OFFLINE' ? 'bg-amber-100 text-amber-900' : 'bg-slate-200 text-slate-700'
                  }`}
              >
            ...
            {/* Division 3: Dual View */}
            <button
              type="button"
              onClick={() => setActiveDivision('ALL')}
              className={`px-4 sm:px-5 py-1.5 rounded-full text-xs font-bold transition-all cursor-pointer flex items-center gap-2 ${activeDivision === 'ALL'
                  ? 'bg-white text-slate-900 shadow-xs font-black scale-[1.01]'
                  : 'text-slate-600 hover:text-slate-900 hover:bg-white/40'
                }`}
            >
              <span>Dual View</span>
